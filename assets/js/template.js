...
//                    }});
    ...
    // Limpa o Bloco após ter sido preenchido
    clearBlock = function (nomeBloco) {
        for (var i = 0; i < that.blockParents.length; i++)
        {
            blocoPai = that.blockParents[i][0];
            blocoFilho = that.blockParents[i][1];
            if (blocoPai === nomeBloco)
            {
                var indexFound = that.hasContent.indexOf(blocoFilho);
                that["BLOCK_" + blocoFilho] = "";
                if (indexFound > -1)
                {
                    that.blockParse[blocoFilho] = "";
    ...
    /**
     * Inicializa a geração do BLoco na estrutura do HTML
     * 
     * @param {type} nomeBloco
     * @returns {undefined}
     */
    ...
    makeBlock = function (nomeBloco) {
        ...
        for (var i = 0; i < that.fields.length; i++)
        {
            ...
            var replaceRegex = new RegExp(field, "g");
            toParse = toParse.replace(replaceRegex, fieldValue);
//            toParse = toParse.replace(field, fieldValue);
    ...
    this.parseJSONObject = function (objRead) {
        ...
        for (var i = 0; i < aData.length; i++)
        {
            chave = aData[i][0];
            valor = aData[i][1];
            if (typeof valor !== "string" & valor !== undefined & valor !== null & typeof valor !== "number")
            {
                this.parseJSONBlock(chave, valor);
            }
            that[chave] = valor;
        }
        i = 1;
    ...
    this.parseJSONBlock = function (blockName, aData) {
        //console.log("BlockName: ", blockName, "dados ", aData);
        for (var i = 0; i < aData.length; i++)
        {
            dados = aData[i];
            chave = aData[i][0];
            ...
            if (typeof dados === "object")
            {
                if (typeof chave === "string")
                {
                    if (Array.isArray(valor))
                    {
                        ...
                        that.parseJSONBlock(chave, Object.entries(valor));
                    } else {
                        that.parseJSONObject(valor);
                    }
                } else {
                    // Preenche os campos com as propriedades do Objeto
                    if (typeof valor === "string")
                    {
                        that[aData[i][0]] = aData[i][1];
                    } else {
                        if (valor !== undefined)
                        {
                            that.parseJSONObject(valor);
                        } else {
                            that.parseJSONObject(dados);
                        }
                    }
                }
            } else
    ...
    this.parseJSON = function (objRead) {
        ...
        for (var i = 0; i < aData.length; i++) {
            if (Array.isArray(aData[i][1]))
            {
                that.parseJSONBlock(aData[i][0], Object.entries(aData[i][1])); // Quando encontra um Objeto JSON ou Array, come�a a fazer a cadeia de blocos
            } else
            {
                that[aData[i][0]] = aData[i][1]; // Preenche os valores dos Campos
            }
        }
        ;
    };
