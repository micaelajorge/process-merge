function calendarCallback(date, month, year, campo)
	{
...
function MostraCalendario(Campo)
	{
	fooCalendar = new dynCalendar('fooCalendar' + Campo, 'calendarCallback', 'images/', false, Campo, 'fooCalendar');
...
* Last Modified..: $Date: 2007/04/07 16:03:10 $
...
*/
	function dynCalendar(objName, callbackFunc, DirImages, writeHTML, campo, Object)
	{
		/**
        * Properties
        */
		...
		this.layerID        = arguments[3] ? arguments[3] : 'dynCalendar_layer_' + objName;
		this.offsetX        = 13;
		this.offsetY        = 0;
...
*/
	function dynCalendar_show()
	{
		...
		monthnames = new Array("Janeiro");
		monthnames.push("Fevereiro")
		monthnames.push("Marco")
		...
		while(i <= numdays){
            ...
            */
			...
			//linkHTML = '<a href="javascript: ' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + ', ' + this.campo + '); ' + this.objName + '._hideLayer()">' + (i++) + '</a>';
			linkHTML = '<a href="javascript: ' + this.callbackFunc + '(' + i + ', ' + (Number(month) + 1) + ', ' + year + ', \'' + this.campo + '\'); ' + this.Object + '._hideLayer()">' + (i++) + '</a>';
			//alert(linkHTML)
...
*/
	function dynCalendar_writeHTML()
	{
		if (is_ie5up || is_nav6up || is_gecko) {
			...
			document.write('<iframe class="dynCalendarIframe" id="iframe' +  this.layerID + '" src="blank.html"> </iframe>');
...
*/
	function dynCalendar_hideLayer()
	{
		if (this._getLayer() != null)
			...
			document.getElementById('iframe' + this.layerID).style.visibility = 'hidden';		
			}
	}
...
*/
	...
	{		
		...
		document.getElementById('iframe' +  this.layerID).style.visibility = 'visible';
	}
...
/*	function dynCalendar_setLayerPosition()
	{
		...
		alert(this._getLayer().style.left);
		document.getElementById('iframe').style.top = (dynCalendar_mouseY + this.offsetY) + 'px';
		document.getElementById('iframe').style.left = (dynCalendar_mouseX + this.offsetX) + 'px';
	}
*/

	function dynCalendar_setLayerPosition()
	{		
		//document.getElementById('iframe'  + this.layerID).style.top = (this.offsetY) + 'px';
		//document.getElementById('iframe'  + this.layerID).style.left = (this.offsetX) + 'px';
		document.getElementById('dynCalendar_layer_fooCalendar' + this.campo).style.top = mousePosY + 'px';
		document.getElementById('dynCalendar_layer_fooCalendar' + this.campo).style.left =  mousePosX + 'px';
		document.getElementById('iframe'  + this.layerID).style.left = mousePosX + 'px';
		document.getElementById('iframe'  + this.layerID).style.top = mousePosY + 'px';
