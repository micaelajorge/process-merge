...
(function ( $, window, document, undefined ) {
        ...
        if(that.options.preview){
            if(!checkFileType(that, files[0])){
                if(typeof that.options.error == "function"){
                    that.options.error($(that.element), "fileNotAllowed", "Tipo de arquivo não permitido! Apenas arquivos tipo ("+that.options.allowedFileTypes+") são permitidos");
                }else
                    alert("Tipo de Arquivo não permitido! Apens arquivos tip ("+that.options.allowedFileTypes+"), são permitidos");
                return;
            }
            if(!checkFileSize(that, files[0])) {
                if(typeof that.options.error == "function"){
                    that.options.error($(that.element), "fileToBig", 'Arquivo muito Grande ('+humanFileSize(files[0].size)+')! Tamanho máximo é ('+that.options.maxFileSize+')');
                }else
                    alert('Arquivo muito grande ('+humanFileSize(files[0].size)+')! Tamanho máximo é ('+that.options.maxFileSize+')');
                return;
            }
        ...
        $(xhr).bind({
            readystatechange: function(){
                ...
                if(this.readyState == 4 && this.status != 200){
                    changeXhrDoneStatus(index);
                    $(".progress.progress-"+index).children().removeClass('active');
                    if(typeof that.options.uploadError  == "function") that.options.uploadError(this, index);
                }
            }
        });
    ...
    function addWrongFileField(that, i, index){
        ...
        $(".error-progress-"+index).css({
            width: that.options.progressBarWidth,
            margin: '20px 0 0 0'
        }).append('<div class="progress-bar progress-bar-danger progress-bar-striped" style="width:100%">Tipo de arquivo ('+extension+') não é permitido</div>');
        $(".error-progress-" + index).wrap('<div class="extra-progress-wrapper"></div>').css("width", that.options.progressBarWidth);
        $(".error-progress-" + index).parent().append('<span title="'+that.options.files[i].name+'">'+fileName+'</span>');
    }
