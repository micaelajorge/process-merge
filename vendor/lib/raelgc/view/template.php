...
namespace raelgc\view {
    ...
    class Template {
        ...
        {
//            $expressaoRegex = "/{(" . self::$REG_NAME . ")((\-\>(" . self::$REG_NAME . "))*)?((\|.*?)*)?}/";
            $expressaoRegex = "/(?<!\\$){(" . self::$REG_NAME . ")((\-\>(" . self::$REG_NAME . "))*)?((\|.*?)*)?}/";
            $r = preg_match_all($expressaoRegex, $content, $m);
            if ($r) {
                for ($i = 0; $i < $r; $i++) {
                    // Object var detected
        ...
        private function substModifiers($value, $exp)
        {
            ...
            for ($i = 1; $i < sizeof($statements); $i++) {
                ...
                // Se é a função default, apenas substitui o valor
                switch ($function) {
                    case "default":
                        if ($value === "") {
                            $value = $temp[1];
...
//                        if (false === strpos($var, "->") && isset($this->values['{' . $exp . '}'])) {
                        if (false === strpos($var, "->")) {
                            if (isset($this->values['{' . $var . '}'])) {
                                $s = str_replace('{' . $exp . '}', $this->substModifiers($this->values['{' . $var . '}'], $exp), $s);
                            } else {
                                $s = str_replace('{' . $exp . '}', $this->substModifiers('', $exp), $s);
                            }
                        }
                    }
                }
