...
{
    edicao = '<button type="button" class="btn btn-primary btn-xs" onclick="jsOpenEditUser(\'' + row.USERID + '\')"><i class=\"fa fa-edit\" title="Editar dados do Usuário"></i></button> ';

    gruposDoUsuario = '<button type="button" class="btn btn-xs btn-warning" onclick="jsEditarGruposUsuario(\'' + row.USERID + '\', \'' + row.USERNAME + '\')"><i class=\"fa fa-users\" title="Editar Grupos do Usuário"></i></button> ';

    senhaDoUsuario = '<button type="button" class="btn btn-xs btn-default" onclick="jsEditarPassword(' + row.USERID + ', \'' + row.USERNAME + '\')"><i class=\"fa fa-lock\" title="Alterar senha do Usuário"></i></button> ';
    fieldValue = edicao + gruposDoUsuario + senhaDoUsuario;
    return fieldValue;
}

function jsInicializacaoPagina()
{
    endApi = SITE_LOCATION + "api/manager/users";
    ...
    datatableUsuarios = tableInit(tabelaDados, endApi, colunas, columnDefs, where, sort, null, null, extraPameters);
}
